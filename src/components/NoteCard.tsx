...
import { Calendar, Check, GripVertical, Pencil, Pin, PinOff, Trash2 } from "lucide-react";
...
export function NoteCard({ note, categories, isNew, dragging, dropPosition, onPointerDown, onPointerMove, onPointerUp, onPointerCancel,
  onToggleCompleted, onTogglePinned, onEdit, onRequestDelete }: Props) {
  ...
  }, [editing, title, details, categoryId, priority, dueAt]);
  ...
  const save = async () => {
    ...
    const unchanged = title.trim() === note.title && details.trim() === (note.details || "") &&
      categoryId === note.categoryId && priority === note.priority && dueAt === (note.dueAt?.slice(0, 10) || "");
    if (unchanged) { setEditing(false); return; }
    setBusy(true);
    if (await onEdit({ title, details, categoryId, priority, dueAt: dueAt || null })) setEditing(false);
    setBusy(false);
  };
  ...
  return <article className={`note-card ${note.completed ? "completed" : ""} ${isNew ? "note-enter" : ""} ${dragging ? "dragging" : ""} ${dropPosition ? `drop-${dropPosition}` : ""}`}
    ...
    <div className="note-main">
      {editing ? <div className="note-edit-form" ref={editorRef}>
        ...
        {(overflows || expanded) && <button className="expand-button" onClick={() => setExpanded(!expanded)}>{expanded ? "收起" : "展开"}</button>}
